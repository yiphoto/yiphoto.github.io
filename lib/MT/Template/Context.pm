...
#
# $Id: Context.pm,v 1.250.2.2 2004/10/21 01:02:40 ezra Exp $
...
sub init_default_handlers {
    ...
    $ctx->register_handler(CGIServerPath => \&_hdlr_cgi_server_path);

    $ctx->register_handler(IfNonEmpty => [ \&_hdlr_if_nonempty, 1 ]);
    $ctx->register_handler(IfNonZero => [ \&_hdlr_if_nonzero, 1 ]);

    $ctx->register_handler(CommenterNameThunk => \&_hdlr_commenter_name_thunk);
    $ctx->register_handler(CommenterName => \&_hdlr_commenter_name);
    $ctx->register_handler(CommenterEmail => \&_hdlr_commenter_email);
    ...
    $ctx->register_handler(EntryBasename => \&_hdlr_entry_basename);
    ...
    $ctx->register_handler(EntryModifiedDate => \&_hdlr_entry_mod_date);
    ...
    $ctx->register_handler(IfAllowCommentHTML => [ \&_hdlr_pass_tokens, 1] );
    $ctx->register_handler(IfRegistrationRequired => [ \&_hdlr_pass_tokens, 1 ]);
    $ctx->register_handler(IfCommentsAllowed => [ \&_hdlr_pass_tokens, 1 ]);
    $ctx->register_handler(IfNeedEmail => [ \&_hdlr_pass_tokens, 1 ]);
    $ctx->register_handler(IfCommenterPending => [ \&_hdlr_pass_tokens, 1 ]);
    $ctx->register_handler(IfDynamicComments => [ \&_hdlr_pass_tokens, 1 ]);

    $ctx->register_handler(TypeKeyToken => \&_hdlr_typekey_token);
    $ctx->register_handler(CommentFields => [ \&_hdlr_comment_fields ]);
    $ctx->register_handler(RemoteSignOutLink => [ \&_hdlr_remote_sign_out_link ]);
    $ctx->register_handler(RemoteSignInLink => [ \&_hdlr_remote_sign_in_link ]);
    ...
    $ctx->register_handler(CommentAuthorIdentity => \&_hdlr_comment_author_identity);
    ...
    $ctx->register_handler(SignOnURL => \&_hdlr_signon_url);
}
...
sub _hdlr_remote_sign_in_link {
    ...
    return "$signon_url$needs_email&t=$rem_auth_token$tk_version&_return=$path$comment_script%3f__mode=handle_sign_in%26$static_arg%26entry_id=$entry_id";
}
...
sub _hdlr_remote_sign_out_link {
    ...
    my $e = $_[0]->stash('entry')
        or return $_[0]->_no_entry_error('MTEntryID');
    my $entry_id = $e->id();
    return "$path$comment_script?__mode=handle_sign_in&$static_arg&entry_id=$entry_id&logout=1";
}
...
sub _hdlr_comment_fields {
    ...
    return unless $blog->allow_reg_comments || $blog->allow_unreg_comments;
...
if (getCookie("commenter_name")) {
    ...
    document.write('<MT_TRANS phrase="You are not signed in. You need to be registered to comment on this site."> <a href="$signon_url$needs_email&t=$rem_auth_token&v=$tk_version&_return=$path$comment_script%3f__mode=handle_sign_in%26$static_arg%26entry_id=$entry_id"><MT_TRANS phrase="Sign in"></a>');
...
if (getCookie("commenter_name")) {
    ...
    document.write('<MT_TRANS phrase="If you have a TypeKey identity, you can "><a href="$signon_url$needs_email&t=$rem_auth_token&v=$tk_version&_return=$path$comment_script%3f__mode=handle_sign_in%26$static_arg%26entry_id=$entry_id"> <MT_TRANS phrase="sign in"></a> <MT_TRANS phrase="to use it here.">');
}
// -->
</script>
HTML
...
sub _hdlr_entry_previous {
    ...
    if ($prev) {
        ...
        $cond{EntryIfCommentsOpen} = $prev->allow_comments &&
            $prev->allow_comments eq '1';
...
sub _hdlr_date {
	...
	my $dt = new DateTime(year => $y, month => $mo, day => $d, 
...
# FIXME: cache this info process-wide.
...

{
	...
	if (MT::ConfigMgr->instance()->ObjectDriver =~ /DBI/
	    ...
	    && ($at ne 'Weekly') && ($at ne 'Individual') && SPEEDUP_ENABLED)
	{
	    my $group_iter;
	    if ($at eq 'Daily') {
...
use MT::Category;
use MT::Entry;
use MT::Promise qw( lazy );

MT::Template::Context->add_container_tag (SubCategories => \&subCats );
MT::Template::Context->add_conditional_tag ( SubCatIsFirst => \&isFirst );
MT::Template::Context->add_conditional_tag ( SubCatIsLast => \&isLast );

MT::Template::Context->add_container_tag ( TopLevelCategories => \&topLevel );

MT::Template::Context->add_container_tag ( ParentCategory => \&parent );
MT::Template::Context->add_container_tag ( ParentCategories => \&tree );
MT::Template::Context->add_container_tag ( TopLevelParent => \&top_parent );

MT::Template::Context->add_tag ( SubCatsRecurse => \&recurse );

MT::Template::Context->add_container_tag ( EntriesWithSubCategories => \&entries );

MT::Template::Context->add_tag ( SubCategoryPath => \&path );

MT::Template::Context->add_conditional_tag ( HasSubCategories => \&hasSubCats );
MT::Template::Context->add_conditional_tag ( HasNoSubCategories => \&hasNoSubCats );
MT::Template::Context->add_conditional_tag ( HasParentCategory => \&hasParent );
MT::Template::Context->add_conditional_tag ( HasNoParentCategory => \&hasNoParent );

MT::Template::Context->add_conditional_tag ( IfIsAncestor => \&is_ancestor );
MT::Template::Context->add_conditional_tag ( IfIsDescendant => \&is_descendant );

sub isFirst { return $_[0]->stash ('subCatIsFirst'); }

sub isLast { return $_[0]->stash ('subCatIsLast'); }
...
# If we find ourselves in a category context
  if ($current_cat = $ctx->stash ('category') || 
...
# then undef
  my $cat = (defined $args->{'category'} 
      && MT::Category->load ({ label => $args->{'category'},
	blog_id => $ctx->stash ('blog_id') })) 
    || $ctx->stash ('category') 
    || $ctx->stash ('archive_category') 
    || undef;
  ...
  if (defined $cat) {
    ...
    $args->{'category'} = join (' OR ', map { $_->label } @cats);
    $saved_stash_entries = $ctx->{__stash}->{entries} if ($save_entries);
    delete $ctx->{__stash}->{'entries'};
  }

  my $res = MT::Template::Context::_hdlr_entries ($ctx, $args, $cond);
  $ctx->{__stash}->{entries} = $saved_stash_entries if ($save_entries && $saved_stash_entries);
  $res;
}
...
sub path {
  ...
  my $builder = $ctx->stash ('builder');
  my $tokens  = $builder->compile ($ctx, "<MTCategoryLabel dirify='1'>");
...
# Return the number of children for the category
  return scalar ($cat->children_categories);
}
