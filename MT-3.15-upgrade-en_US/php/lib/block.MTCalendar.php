...
function smarty_block_MTCalendar($args, $content, &$ctx, &$repeat) {
    $local_vars = array('cal_entries','cal_day','cal_pad_start','cal_pad_end','cal_days_in_month','cal_prefix','cal_left','CalendarDay','CalendarWeekHeader','CalendarWeekFooter','CalendarIfEntries','CalendarIfNoEntries','CalendarIfToday','CalendarIfBlank','entries','current_timestamp');
    ...
    if ($day <= $pad_start + $days_in_month + $pad_end) {
        ...
        if (!$is_padding) {
            ...
            $no_loop = 0;
            if (count($left)) {
                if (substr($left[0]['entry_created_on'], 0, 8) == $this_day) {
                    $entries = $left;
                    $left = array();
                } else {
                    $no_loop = 1;
            ...
            if (!$no_loop && count($iter)) {
                while ($entry = array_shift($iter)) {
                    $e_day = substr($entry['entry_created_on'], 0, 8);
                    if ($e_day != $this_day) {
                        $left[] = $entry;
                        break;
                    }
