...
#
# $Id: Blog.pm,v 1.54.4.1 2004/12/16 02:07:40 bchoate Exp $
...
__PACKAGE__->install_properties({
    columns => [
        ...
        'archive_type_preferred', 'old_style_archive_links',
        'site_path', 'site_url',
        ...
        'moderate_unreg_comments', 'allow_unreg_comments', 'allow_reg_comments', 
	'manual_approve_commenters', 'require_comment_emails',
        'ping_weblogs', 'mt_update_key', 'language', 'welcome_msg',
        'google_api_key', 'email_new_pings', 'ping_blogs', 'ping_technorati',
        'ping_others',
        'autodiscover_links', 'sanitize_spec', 'cc_license',
        'is_dynamic', 'remote_auth_token', 'children_modified_on',
...
# The effective remote_auth token is the remote_auth token on this weblog,
# or any remote_auth token of any author who has permission on this weblog.
...
sub effective_remote_auth_token {
	...
	my @authors = MT::Author->load({},
				       {join => ['MT::Permission',
						 ...
						 {blog_id => $blog->id}, {}]});
	for my $au (@authors) {
	    return $au->remote_auth_token if $au->remote_auth_token
	}
    }
}
...
without a validated email/password pair) are allowed. This option
being false forces anonymous comments to be disallowed; but the
allow_anon_comments flag need not reflect that.
