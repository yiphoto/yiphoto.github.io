...
class statistics {
	function browser_stats($database, $dbprefix, $statisticshtml, $task){
		$query = "SELECT name, count FROM ".$dbprefix."counter WHERE type='browser' ORDER BY count DESC";
	...
	function os_stats($database, $dbprefix, $statisticshtml, $task){
		$query = "SELECT name, count FROM ".$dbprefix."counter WHERE type='os' ORDER BY count DESC";
		$result = $database->openConnectionWithReturn($query);
		$i = 0;
		$sum = 0;
		while ($row = mysql_fetch_object($result)){
