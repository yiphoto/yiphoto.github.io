...
class menusections {
	function viewMenuItems($database, $dbprefix, $menusectionshtml, $option){
		$query="select id, name, link, inuse, contenttype, editor, access from ".$dbprefix."menu where menutype='mainmenu' and componentid='0' order by ordering";
		...
		$menusectionshtml->showMenusections($itemid, $itemName, $type, $status, $option, $editor, $GNameSel);
	}

	function addMenusection($database, $dbprefix, $menusectionshtml, $option){
		...
		$menusectionshtml->addMenusection($option, $Gid, $Gname);
	}

	function addStep2($database, $dbprefix, $menusectionshtml, $option, $ItemName, $ItemType, $access){
		$query="select id from ".$dbprefix."menu where menutype='mainmenu' and name='$ItemName' and componentid=0";
		$result=$database->openConnectionWithReturn($query);
		if (mysql_num_rows($result)> 0){
			...
			if ($ItemType=="Mambo"){
				...
				$menusectionshtml->addMamboStep2($option, $ItemName, $moduleid, $modulename, $access);
			}else if ($ItemType=="Own"){
				$menusectionshtml->addOwnStep2($option, $ItemName, $ItemType, $access);
			}else if ($ItemType=="Web"){
				$menusectionshtml->addWebStep2($option, $ItemName, $ItemType, $access);
			}
		}
	}

	function addStep3($database, $dbprefix, $menusectionshtml, $option, $ItemName, $PageSource, $text_editor, $access){
		if ($PageSource=="Type"){
			$menusectionshtml->addTypeStep3($option, $ItemName, $text_editor, $access);
		}else if ($PageSource=="Link"){
			$query="select Max(ordering) As LastOrder from ".$dbprefix."menu where menutype='mainmenu' and componentid='0'";
			$result=$database->openConnectionWithReturn($query);
			list ($lastOrder)=mysql_fetch_array($result);
			mysql_free_result($result);
			$nextOrder=$lastOrder+1;
			$query="insert into ".$dbprefix."menu (menutype, name, link, inuse, componentid, ordering, contenttype) values ('mainmenu', '$ItemName', '', '0', 0, $nextOrder, 'typed')";
			$database->openConnectionNoReturn($query);
			$query="select id from ".$dbprefix."menu where menutype='mainmenu' and name='$ItemName' and componentid=0";
			$result=$database->openConnectionWithReturn($query);
			list ($Itemid)=mysql_fetch_array($result);
			$menusectionshtml->addLinkStep3($option, $ItemName, $Itemid, $access);
		}
	}

	function saveMenusection($database, $dbprefix, $ItemName, $pagecontent, $Weblink, $moduleID, $option, $heading, $browserNav, $access){
		if ((trim($moduleID)=="") && (trim($pagecontent)=="") && (trim($Weblink)=="")){
			...
			$query="insert into ".$dbprefix."menu (menutype, name, link, inuse, componentid, ordering, contenttype, browserNav, access) values ('mainmenu', '$ItemName', '$link', '0', 0, $nextOrder, '$contenttype', $browserNav, '$access')";
	...
	function editMenusection($database, $dbprefix, $menusectionshtml, $option, $Itemid, $myname, $text_editor){
		if ($Itemid == ""){
			print "<SCRIPT> alert('Select a menu item to edit'); window.history.go(-1);</SCRIPT>\n";
		}

		$query="select name, link, inuse, contenttype, ordering, checked_out, editor, browserNav from ".$dbprefix."menu where id='$Itemid'";
		$result= $database->openConnectionWithReturn($query);
		...
		$query="select Max(ordering) As maxOrder from ".$dbprefix."menu where menutype='mainmenu' and componentid='0'";
		$result=$database->openConnectionWithReturn($query);
		...
		$menusectionshtml->editMenusection($Itemid, $ItemName, $pagecontent, $link, $fileEdit, $filecontent, $mamboEdit, $moduleid, $modulename, $moduleidlist, $modulenamelist, $option, $order, $maxOrder, $myname, $heading, $browserNav, $text_editor, $Gid, $Gname, $GIDSel);
	}

	function saveEditMenusection($database, $dbprefix, $menusectionshtml, $option, $ItemName, $pagecontent, $filecontent, $Itemid, $order, $origOrder, $myname, $Weblink, $link2, $heading, $browserNav, $access){
		if ($ItemName == ""){
			print "<SCRIPT> alert('Please enter a page name'); window.history.go(-1); </SCRIPT>\n";
		}else{
			$query="select id from ".$dbprefix."menu where menutype='mainmenu' and name='$ItemName' and componentid='0' and id!='$Itemid'";
			$result=$database->openConnectionWithReturn($query);
			if (mysql_num_rows($result)> 0){
				...
				if ($type=="file"){
					if (trim($filecontent)!=""){
						...
						$query= "update ".$dbprefix."menu set name='$ItemName' where id='$Itemid'";
						$database->openConnectionNoReturn($query);
					}
				}else if ($type=="typed"){
					// $pagecontent = ereg_replace("\x0D\x0A", "<BR>", $pagecontent);
					...
					if (trim($Weblink)==""){
						...
						$query= "update ".$dbprefix."menu set name='$ItemName', link='$Weblink', browserNav=$browserNav where id='$Itemid'";
						$database->openConnectionNoReturn($query);
					}
				}
				else {
				...
				if ($order > $origOrder){
					$query = "SELECT id FROM ".$dbprefix."menu WHERE ordering <= $order AND ordering > $origOrder AND menutype='mainmenu' AND componentid='0' ORDER BY ordering";
					$result = $database->openConnectionWithReturn($query);
					while ($row = mysql_fetch_object($result)){
						$changeid = $row->id;
				...
				$query = "SELECT id FROM ".$dbprefix."menu WHERE id='$Itemid' AND checked_out=1";
				$result = $database->openConnectionWithReturn($query);
				if (mysql_num_rows($result) > 0){
	...
	function saveFileUpload($database, $dbprefix, $option, $userfile, $userfile_name, $Itemid){
		...
		if (eregi(".htm", $userfile_name)){
			...
			if (!move_uploaded_file($userfile, $base_Dir.$userfile_name)){
				...
				$query="update ".$dbprefix."menu set link='uploadfiles/$Itemid/$userfile_name', inuse='0', contenttype='file' where id='$Itemid'";
				$database->openConnectionNoReturn($query);
				echo "<SCRIPT>document.location.href='index2.php?option=$option&task=edit&cid%5B%5D=$Itemid'</SCRIPT>";
			}
		}else{
	...
	function publishMenusection($database, $dbprefix, $option, $Itemid, $cid){
		if (trim($Itemid)!=""){
	...
	function unpublishMenusection($database, $dbprefix, $option, $Itemid, $cid){
		if (trim($Itemid)!=""){
	...
	function removeMenusection($database, $dbprefix, $option, $cid){
		if (count($cid) == 0){
